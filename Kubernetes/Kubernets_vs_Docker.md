...
<img src="./Docker vs Kubernetes.png" width="100%" />

✅ Docker solves containerization; Kubernetes solves container orchestration. Docker tells us how to package and run an application, while Kubernetes manages where containers run, how many should run, how they communicate, how they recover from failures, and how they are deployed and scaled across a cluster.
...
| Feature                       | Plain Docker — Single Host                                                                                                               | Kubernetes — Cluster Orchestration                                                                                                                            |
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Scope**                     | Runs containers on a single host by default.                                                                                             | Orchestrates workloads across multiple Nodes in a cluster.                                                                                                    |
| **Desired State**             | You generally manage containers explicitly.                                                                                              | You declare the desired state; Kubernetes continuously reconciles actual state toward it.                                                                     |
| **Scaling**                   | Usually manual unless you build additional automation.                                                                                   | Can scale automatically using mechanisms such as **HPA**; otherwise replica changes can be manual.                                                            |
| **Self-Healing**              | Docker can restart containers according to restart policies, but a single-host failure takes the workload down.                          | Controllers can recreate failed Pods and schedule replacements on healthy Nodes, assuming capacity and constraints allow it.                                  |
| **Node Failure**              | If the host fails, containers on that host become unavailable.                                                                           | Workloads can be rescheduled onto healthy Nodes.                                                                                                              |
| **Rolling Updates**           | You need to implement deployment/update strategy yourself or use additional tooling.                                                     | Deployments provide rolling-update capabilities and can maintain application availability during updates when configured appropriately.                       |
| **Load Balancing**            | Docker provides basic container networking/port publishing, but production traffic distribution usually needs additional infrastructure. | Kubernetes provides Services for stable networking and traffic distribution among Pods; external traffic may additionally use Ingress/Gateway/load balancers. |
| **Service Discovery**         | Limited/basic Docker networking on a host.                                                                                               | Built-in Kubernetes Service/DNS mechanisms provide service discovery.                                                                                         |
| **Multi-Host Management**     | Not the primary purpose of plain Docker.                                                                                                 | One of Kubernetes' core purposes.                                                                                                                             |
| **Infrastructure Complexity** | Relatively simple.                                                                                                                       | Much more complex, especially when self-managed.                                                                                                              |
